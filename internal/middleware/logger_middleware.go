...
func LoggerMiddleware(logger *zerolog.Logger) gin.HandlerFunc {
	return func(ctx *gin.Context) {
		...
		logEvent.
			Str("trace_id", loggers.GetTraceID(ctx.Request.Context())).
			...
			Interface("request_body", senitizeRequest(requestBody, sensitiveFields)).
...
func senitizeRequest(data map[string]any, sensitiveKeys []string) map[string]any {
	...
	for key, val := range data {
		...
		if shouldMask {
			...
			switch v := val.(type) {
			case map[string]any:
				sensitized[key] = senitizeRequest(v, sensitiveKeys)
			case []any:
				var senitizedSlice []any
				for _, item := range v {
					if m, ok := item.(map[string]any); ok {
						senitizedSlice = append(senitizedSlice, senitizeRequest(m, sensitiveKeys))
					} else {
						senitizedSlice = append(senitizedSlice, item)
					}
				}
				sensitized[key] = senitizedSlice
			default:
				sensitized[key] = val
			}
		}
